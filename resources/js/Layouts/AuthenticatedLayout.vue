...
<template>
    <div>
        ...
        >
            <div
                v-if="showPendingTasksToast && pendingTasksNotice"
            ...
            <nav class="bg-white shadow-sm border-b border-gray-100 sticky top-0 z-40">
                        ...
                        <div class="flex">
                            ...
                            <div class="flex shrink-0 items-center">
                                <Link :href="route('dashboard')" class="flex items-center gap-3 group">
                                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center shadow-md group-hover:shadow-lg transition-all duration-300 transform group-hover:scale-105">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />
                                        </svg>
                                    </div>
                                    <div>
                                        <h1 class="text-xl font-black tracking-tight text-gray-900 leading-none">KÖKSAN</h1>
                                        <span class="text-[10px] font-bold text-indigo-600 tracking-widest uppercase">Süreç Portalı</span>
                                    </div>
                                </Link>
                            </div>
                            ...
                            <div class="hidden sm:-my-px sm:ml-10 sm:flex sm:space-x-8">
                                ...
                                <NavLink v-if="hasPermission('start_processes')" :href="route('processes.index')" :active="route().current('processes.index')">
                                    🚀 Yeni Talep Başlat
                                </NavLink>
                                ...
                                <NavLink :href="route('processes.history')" :active="route().current('processes.history')">
                                    📁 Benim Taleplerim
                                </NavLink>
                                
                                <NavLink v-if="hasPermission('processes.view_department')" :href="route('processes.department')" :active="route().current('processes.department')">
                                    Bölüm Süreçleri
                                </NavLink>
                            </div>
                        </div>
                        ...
                        <div class="hidden sm:flex sm:items-center sm:ml-6 space-x-4">
                            ...
                            <div class="relative">
                                <Dropdown align="right" width="80">
                                    ...
                                    <template #content>
                                        ...
                                        <div v-if="recentNotifications.length === 0" class="px-4 py-6 text-center text-sm text-gray-500">
                                            Yeni bildirim yok.
                                        </div>
                                        ...
                                        <div v-else class="max-h-[70vh] overflow-y-auto w-80 sm:w-96">
                                            ...
                                            >
                                                ...
                                                <p class="text-sm font-bold text-gray-900 whitespace-normal break-words leading-tight mt-1">
                                                    {{ notification.title }}
                                                </p>
                                                <p class="mt-1.5 text-xs text-gray-600 whitespace-normal break-words leading-relaxed">
                                                    {{ notification.body }}
                                                </p>
                                            </button>
                                        </div>
                            ...
                            <div class="relative" v-if="hasPermission('view_admin_panel')">
                                <Dropdown align="right" width="56">
                                    <template #trigger>
                                        <button class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-bold rounded-full text-indigo-700 bg-indigo-50 hover:bg-indigo-100 hover:text-indigo-900 focus:outline-none transition-all duration-200">
                                    ...
                                    <template #content>
                                        <div class="px-3 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">Ayarlar</div>
                                        <DropdownLink v-if="hasPermission('view_admin_panel')" :href="route('admin.settings.index')">Sistem Ayarları</DropdownLink>
                                        <DropdownLink v-if="hasPermission('manage_users')" :href="route('admin.users.index')">Kullanıcılar</DropdownLink>
                                        <DropdownLink v-if="hasPermission('manage_roles')" :href="route('admin.roles.index')">Roller & Yetkiler</DropdownLink>
                                        <div class="border-t border-gray-100 my-1"></div>
                                        <div class="px-3 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">Süreç Mimarisi</div>
                                        <DropdownLink v-if="hasPermission('create_forms')" :href="route('form-templates.index')">Form Şablonları</DropdownLink>
                                        <DropdownLink v-if="hasPermission('manage_workflows')" :href="route('workflows.index')">Akış Tasarımcısı</DropdownLink>
                                        <DropdownLink v-if="hasPermission('manage_departments')" :href="route('admin.directorates.index')">Direktörlükler</DropdownLink>
                                        <DropdownLink v-if="hasPermission('manage_departments')" :href="route('admin.departments.index')">Departmanlar</DropdownLink>
                                        <DropdownLink v-if="hasPermission('manage_workflows')" :href="route('admin.workflow-categories.index')">Süreç Kategorileri</DropdownLink>
                            ...
                            <div class="relative ml-3 border-l border-gray-200 pl-4">
                                <Dropdown align="right" width="48">
                                    <template #trigger>
                                        <button class="flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 focus:outline-none transition duration-150 ease-in-out group">
                                            ...
                                            <div class="text-left hidden lg:block">
                                                ...
                                                <div class="text-xs text-gray-500">{{ user.roles?.[0]?.name || 'Personel' }}</div>
                                            </div>
                                    ...
                                    <template #content>
                                        ...
                                        <DropdownLink :href="route('logout')" method="post" as="button" class="text-red-600 hover:bg-red-50 hover:text-red-700 font-medium">Güvenli Çıkış</DropdownLink>
                                    </template>
                                </Dropdown>
                            </div>
                        </div>
                ...
                <div :class="{'block': showingNavigationDropdown, 'hidden': !showingNavigationDropdown}" class="sm:hidden border-t border-gray-200">
                    <div class="pt-2 pb-3 space-y-1">
                        <ResponsiveNavLink :href="route('dashboard')" :active="route().current('dashboard')">🏠 Ana Sayfa</ResponsiveNavLink>
                        
                        <ResponsiveNavLink v-if="hasPermission('start_processes')" :href="route('processes.index')" :active="route().current('processes.index')">🚀 Yeni Talep Başlat</ResponsiveNavLink>
                        ...
                        <ResponsiveNavLink :href="route('processes.history')" :active="route().current('processes.history')">📁 Benim Taleplerim</ResponsiveNavLink>

                        <ResponsiveNavLink :href="route('notifications.index')" :active="route().current('notifications.*')">
                            🔔 Bildirimler
                        </ResponsiveNavLink>
                        
                        <ResponsiveNavLink v-if="hasPermission('processes.view_department')" :href="route('processes.department')" :active="route().current('processes.department')">Bölüm Süreçleri</ResponsiveNavLink>
                    </div>

                    <!-- Responsive Admin Links -->
                    <div v-if="hasPermission('view_admin_panel')" class="pt-4 pb-1 border-t border-gray-200 bg-gray-50">
                        <div class="px-4 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">⚙️ Sistem Yönetimi</div>
                        <ResponsiveNavLink v-if="hasPermission('view_admin_panel')" :href="route('admin.settings.index')">Sistem Ayarları</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('manage_users')" :href="route('admin.users.index')">Kullanıcılar</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('manage_roles')" :href="route('admin.roles.index')">Roller & Yetkiler</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('create_forms')" :href="route('form-templates.index')">Form Şablonları</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('manage_workflows')" :href="route('workflows.index')">Akış Tasarımcısı</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('manage_departments')" :href="route('admin.directorates.index')">Direktörlükler</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('manage_departments')" :href="route('admin.departments.index')">Departmanlar</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('manage_workflows')" :href="route('admin.workflow-categories.index')">Süreç Kategorileri</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('create_forms')" :href="route('admin.form-categories.index')">Form Kategorileri</ResponsiveNavLink>
                        <ResponsiveNavLink v-if="hasPermission('view_admin_panel')" :href="route('admin.tree-types.index')">Şema Tasarımcısı</ResponsiveNavLink>
                    ...
                    <div class="pt-4 pb-1 border-t border-gray-200">
                        ...
                        <div class="mt-3 space-y-1">
                            ...
                            <ResponsiveNavLink :href="route('logout')" method="post" as="button" class="text-red-600">Güvenli Çıkış</ResponsiveNavLink>
                        </div>
                    </div>
                </div>
            </nav>
