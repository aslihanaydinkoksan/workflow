...
const a4Document = ref(null);

const exportToPDF = () => {
    if (!a4Document.value) return;
    const opt = {
        margin:       0.5,
        filename:     `${props.template?.name || 'Form'}.pdf`,
        image:        { type: 'jpeg', quality: 0.98 },
        html2canvas:  { scale: 2 },
        jsPDF:        { unit: 'in', format: 'a4', orientation: 'portrait' }
    };
    html2pdf().set(opt).from(a4Document.value).save();
};

const exportToExcel = () => {
    if (!a4Document.value) return;
    
    let html = a4Document.value.outerHTML;
    // Basit HTML to Excel dönüşümü
    ...
    let ctx = { worksheet: props.template?.name || 'Form', table: html };
    
    let link = document.createElement("a");
    link.download = `${props.template?.name || 'Form'}.xls`;
...
<template>
    <div class="form-renderer-wrapper w-full">
        ...
        <div v-if="template" class="flex justify-end space-x-3 mb-4 print:hidden">
            <button @click="exportToExcel" type="button" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md shadow-sm">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Excel İndir
            </button>
            <button @click="exportToPDF" type="button" class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-md shadow-sm">
        ...
        <div ref="a4Document" :class="template ? 'bg-white shadow-xl mx-auto p-8 border border-gray-200 print:shadow-none print:border-none w-full' : ''" :style="template ? 'min-height: 29.7cm;' : ''">
            
            <!-- Köksan Doküman Başlığı (Sadece template varsa) -->
            <div v-if="template" class="mb-8 border-2 border-black w-full flex flex-col md:flex-row">
                <!-- Logo Alanı -->
                <div class="border-b-2 border-black md:border-b-0 md:border-r-2 border-black p-2 flex items-center justify-center min-w-[200px]">
                    <img v-if="appLogo" :src="appLogo" alt="App Logo" :style="{ width: (template.logo_width || 4.5) + 'cm', height: (template.logo_height || 1.5) + 'cm', objectFit: 'contain' }">
                    <div v-else class="text-xs text-gray-400 border border-dashed border-gray-300 p-4 flex items-center justify-center text-center" :style="{ width: (template.logo_width || 4.5) + 'cm', height: (template.logo_height || 1.5) + 'cm' }">
                        Logo Yok
                    </div>
                </div>
                
                <!-- Başlık Alanı -->
                <div class="flex-1 flex flex-col items-center justify-center border-b-2 border-black md:border-b-0 md:border-r-2 border-black p-4 text-center">
                ...
                <div class="w-full md:w-64 flex flex-col text-xs font-semibold">
                    ...
                    <div class="flex border-b border-black">
                        ...
                        <div class="w-1/2 p-1.5">{{ template.publish_date ? template.publish_date.split('T')[0] : '-' }}</div>
                    </div>
                    <div class="flex border-b border-black">
                        <div class="w-1/2 p-1.5 border-r border-black">Rev. No / Tarih</div>
                        <div class="w-1/2 p-1.5">{{ template.revision_no || '0' }} / {{ template.revision_date ? template.revision_date.split('T')[0] : '-' }}</div>
                    </div>
                    <div class="flex">
            ...
            <div v-else class="relative">
                ...
                >
                    ...
                    <option v-for="(opt, i) in getOptions(element.options)" :key="i" :value="opt.trim ? opt.trim() : opt">{{ opt.trim ? opt.trim() : opt }}</option>
                </select>
                ...
                <div v-else-if="element.type === 'radio'" class="space-y-2 mt-2">
                    <div v-for="(opt, i) in getOptions(element.options)" :key="i" class="flex items-center">
                        ...
                        />
                        <label class="ml-2 block text-sm" :class="(disabled || !!element.data_bind || element.is_readonly) ? 'text-gray-500' : 'text-gray-700'">
                ...
                <div v-else-if="element.type === 'file'">
                    <input 
                        type="file" 
                        :required="element.required"
                        :disabled="disabled || element.is_readonly"
                ...
                <div v-else-if="element.type === 'checkbox'" class="flex items-center mt-2">
                    ...
                    />
                    <span class="ml-2 text-sm" :class="(disabled || !!element.data_bind || element.is_readonly) ? 'text-gray-500' : 'text-gray-700'">Evet/Onaylıyorum</span>
                </div>
                
                <p v-if="element.description && element.type !== 'header'" class="mt-1 text-xs text-gray-500">{{ element.description }}</p>

                <p v-if="element.data_bind" class="mt-1.5 text-[11px] text-indigo-600 font-semibold flex items-center gap-1">
        ...
        <div v-if="elements.length === 0" class="col-span-12 text-sm text-gray-500 italic p-6 bg-gray-50 rounded-lg border-2 border-dashed text-center">
